...
const Booking = () => {
    ...
    const handleDateChange = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/timeslot/doctor?doctorId=${doctorId}&date=${selectedDate}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
            const data = await response.json();
            setTimeSlots(data); // Update time slots for the dropdown
            console.log(data);
        } catch (error) {
            ...
            >
                ...
                <section className="flex items-center justify-between w-full p-4 lg:flex-row flex-col gap-12">
                    <div className="flex flex-col gap-12 lg:w-[50%] w-full lg:items-start lg:order-1 order-2">
                        ...
                        {date && 
                            <div className="flex flex-col lg:items-start items-center gap-2 lg:w-full ">
                                <p className="text-2xl font-medium">Select Available Time Slot</p>
                                <select value={timeSlot} onChange={handleTimeSlotChange} className="border-dark border-2 p-4 lg:w-[70%] w-[90%] text-dark rounded-lg text-lg font-medium">
                                    {timeSlots.map((slot) => (
                ...
                <div className="flex flex-col px-4 gap-2 lg:items-start items-center w-full hidden">

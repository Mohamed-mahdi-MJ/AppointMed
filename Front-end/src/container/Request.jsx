...
const Request = () => {
    ...
    const handleCancel = async (appointmentId) => {
        try {
            await fetch(`http://localhost:8080/api/appointment/${appointmentId}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                ...
                >
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="2xl:flex hidden items-center text-xl font-bold w-full px-4">
                            <p className="flex-1">{role === "PATIENT" ? "Doctor" : "Patient"} Name</p>
                        ...
                        {appointments.map((appointment, index) => (
                            <div key={index} className="flex items-center bg-dark/5 rounded-lg px-4 py-4 gap-4 w-full">
                                <p className="flex-1">{role === "PATIENT" ? appointment.doctor.name : appointment.patient.name}</p>
                                <div className="flex-1 flex flex-col">
                                    <p>{appointment.date}</p>
                                    <p>{appointment.timeSlot.time}</p>
                                </div>
                                ...
                                <div className="flex-1 flex gap-2 justify-end">
                                    ...
                                    {role === "DOCTOR" && appointment.status === "SCHEDULED" && (
                                        <>
                                            <button
                                                disabled
                                                className="bg-gray-400 text-white py-1 px-2 rounded-lg cursor-not-allowed hidden"
                                            >
                                                Approve
                                            </button>
                                            <button

...
const Staff = () => {
    ...
    const { city } = useParams(); // Capture department from the URL
    ...
    useEffect(() => {
        ...
        const fetchStaffs = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/doctor/${city}`, {
                    headers: {
    ...
    return (
        <section className="flex flex-col min-h-screen gap-12 bg-light dark:bg-dark">
            ...
            >
                <h1 className="md:text-[4rem] sm:text-[3rem] ssm:text-[2.5rem] font-bold -tracking-tight text-center px-8">Meet Our Esteemed Doctors</h1>
                ...
                <div className="grid 2xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 ssm:grid-cols-1 items-center gap-12 mt-8 ">
                    {staffs.map((item, index) => (
                        <div className="bg-gray-100 dark:bg-black/30 rounded-lg flex flex-col px-12 py-6 items-center gap-2">
                            <img src="/doctor.png" alt="profile" className="w-[12rem] h-[12rem] rounded-lg" />
                            <p className="font-semibold text-lg">{item.name}</p>
                            <p className="text-lg font-medium">{item.specialization}</p>
                            <p className="text-lg font-medium">{item.clinicName}</p>
